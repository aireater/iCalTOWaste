...
def WriteCal():
    ...
    for line in data:
        ...
        [Calendar, Day, GreenBin, Garbage, Recycling,
            Yardwaste, ChristmasTree] = line
        ...
        if ChristmasTree != "0":
            ...
            description = [
                "Recycling and Green Bin - More information on what can be recycled click here: http://www.toronto.ca/garbage/bluebin.htm"]
...
def WriteIcs():
    ...
    for line in data2:
        [Calendar, WeekStarting, GreenBin, Garbage,
            Recycling, Yardwaste, ChristmasTree] = line
        if Calendar == "Calendar":
            print "#skipping header"
            # skip first line as that is header
            data2.next()
        ...
        if ChristmasTree != "0":
            subject = "Christmas Tree/Garbage Day"
            description = (
                "Garbage and Green Bin waste, Christmas tree collection occurs Today. "
                "When placing your tree out for collection, please remove all decorations, "
                "tinsel, etc and do not place out in any type of bag")
            url = ""
        elif Recycling != "0":
            subject = "Recycling Day"
            description = "Recycling and Green Bin"
            url = "http://www.toronto.ca/garbage/bluebin.htm"
        elif Garbage != "0" and Yardwaste != "0":
            subject = "Garbage/Yard Waste Day"
            description = "Garbage, Yard and Green Bin"
            url = "http://app.toronto.ca/wes/winfo/search.do"
        elif Garbage != "0":
            subject = "Garbage Day"
            description = "Garbage and Green Bin"
            url = "http://app.toronto.ca/wes/winfo/search.do"
